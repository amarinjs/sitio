...
Lets try to issue snmpwalk again.

<div>{%- include extensions/youtube.html id='fwWigkQmLgI' -%}</div>

Note on the linux terminals, how the data is being polled at the given rate. On the cisco router, conformed and exceeded traffic are being accounted for.
